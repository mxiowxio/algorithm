...
public class _4_寻找两个正序数组的中位数 {
    ...
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        int n = nums1.length;
        int m = nums2.length;
        // 因为数组是从索引0开始的，因此我们再这里必须+1，即索引（K+1)的数，才是第k个数
        int left = (n + m + 1) >> 1;
        int right = (n + m + 2) >> 1;
    ...
    private int getKth(int[] nums1, int start1, int end1, int[] nums2, int start2, int end2, int k) {
        // 因为索引和算数不同6 - 0 = 6,但是是有7个数的，因为end出事就是数组长度-1构成的。
        // 最后len代表当前数组（也可能是经过递归排除后的数组），符合当前元素的个数
        int len1 = end1 - start1 + 1;
        int len2 = end2 - start2 + 1;
        // 让len1的长度小于len2，这样就能保证如果有数组空了，一定是len1
        // 就是如果len1的长度小于len2，把getKth()中参数互换位置，即原来的len2就变成了len1，即len1永远小于len2
        if (len1 > len2) {
            return getKth(nums2, start2, end2, nums1, start1, end1, k);
        }
        // 如果一个数组中没有了元素，那么即从剩余数组nums2的其实start开始加k再-1
        // 因为k代表个数，而不是索引，那么从nums2后再找k个数，那个就是start2 + k - 1 索引处就行了。
        ...
        // 为了防止数组长度小于k/2，每次比较都会从当前数组所生长度和k/2作比较，取其中的小的（如果取大的，数组就会越界）
